...
class HardwiredAgent(CaptureAgent):
    ...
    #this gets the distance from a given position to the enemy specified by enemy index
    def getPosDistToEnemy(self, cpos, enemyIndex):
        if enemyIndex in self.knownEnemies:
            return self.getMazeDistance(cpos, self.knownEnemies[enemyIndex])
        else:
    ...
    def getDistanceToEnemy(self, gameState, enemyIndex):
        return self.getPosDistToEnemy(self.getMyPos(gameState), enemyIndex)
